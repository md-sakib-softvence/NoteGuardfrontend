...
import { useGetSingleNoteQuery, useUpdateNoteMutation } from "@/store/Api/Note/note.api";
...
export default function EditNotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: noteData, isLoading: isFetching } = useGetSingleNoteQuery(noteId);
  ...
  if (isFetching) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-sm text-gray-500">Loading your note...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Edit Note</h1>
        </div>
        <button
          onClick={handleSave}
          disabled={isUpdating}
