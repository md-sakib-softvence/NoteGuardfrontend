...
import { useGetUsersByInterestsQuery } from "@/store/Api/User/user.api";

const fallbackInterests = [
  { id: "Technology", name: "Technology", count: 452, percentage: 35 },
  { id: "Business", name: "Business", count: 312, percentage: 25 },
  { id: "Design", name: "Design", count: 204, percentage: 16 },
  { id: "Personal Development", name: "Personal Development", count: 180, percentage: 14 },
  { id: "Health & Fitness", name: "Health & Fitness", count: 100, percentage: 10 },
];

export default function UsersByInterestsPage() {
  const { data } = useGetUsersByInterestsQuery();

  const apiData = data?.data;
  let displayInterests = fallbackInterests;

  if (apiData && apiData.length > 0) {
    const totalCount = apiData.reduce((acc, item) => acc + item.count, 0);
    displayInterests = apiData.map((item) => ({
      id: item._id,
      name: item._id,
      count: item.count,
